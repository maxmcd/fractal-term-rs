...
impl JuliaView {
    pub fn new(matrix_w: usize, matrix_h: usize, specs: FractalSpecs) -> Self {
        JuliaView {
            ...
            julia_coord_animator: Animator::<Vector2f>::new(
    ...
    fn start_julia_coord_animator(&mut self, current: Complex64, index: usize) {
        ...
        let current2 = Vector2f {
            x: current.re,
            y: current.im,
        }; // convert for Animator
        self.julia_coord_animator.value = current2;

        let target = self.julia_coordlist.get(index);
        let target2 = Vector2f {
            x: target.re,
            y: target.im,
        }; // convert for Animator
        let anim = Anim::Target {
            target: target2,
            coefficient: constants::TARGET_COEF * 1.0,
            epsilon: None,
        };
...
impl View for JuliaView {
    ...
    fn do_dirty_fractal_check(&mut self) -> bool {
        let v = vec![
            ...
            self.julia_coord_animator.value.x,
            self.julia_coord_animator.value.y,
        ];
