...
impl MandelView {
    pub fn new(matrix_w: usize, matrix_h: usize, specs: FractalSpecs) -> Self {
        MandelView {
            ...
            exposure_floor_animator: Animator::<f64>::new(
                0.0,
                Anim::Target {
                    target: 0.0,
                    coefficient: 0.1,
            ...
            exposure_ceil_animator: Animator::<f64>::new(
                specs.max_val as f64,
                Anim::Target {
                    target: specs.max_val as f64,
                    coefficient: 0.1,
                    epsilon: Some(0.01),
                },
            ),
    ...
    fn start_mandel_coord_anim(&mut self, index: usize) {
        ...
        self.position_animator().set_anim(Anim::Target {
            target: Vector2f { x: 0.0, y: 0.0 },
            coefficient: constants::TARGET_COEF * 0.4,
            epsilon: None,
        });
        ...
        self.width_animator.set_anim(Anim::Target {
            target: dw,
            coefficient: constants::TARGET_COEF * 1.5,
            epsilon: Some(0.003),
        });
    }
    ...
    fn start_mandel_coord_anim_2(&mut self) {
        ...
        self.position_animator().set_anim(Anim::Target {
            ...
            coefficient: constants::TARGET_COEF * 0.55,
            epsilon: None,
        });
        let target_w = 1.0 / (poi.2 / self.specs.default_width);
        self.width_animator().set_anim(Anim::Target {
            target: target_w,
            coefficient: constants::TARGET_COEF * 0.5,
            epsilon: None,
        }); // important: no epsilon
    }
}
...
impl View for MandelView {
    ...
    fn do_dirty_fractal_check(&mut self) -> bool {
        let v = vec![
            self.position_animator.value.x,
            self.position_animator.value.x,
            self.width_animator.value,
            self.rotation_animator.value,
        ];
            ...
            FractalType::Mandelbrot => {
                if self.coord_anim_phase == 1 {
                    ...
                    match self.width_animator.anim() {
                        &Anim::Target { target, .. } => {
                            // end condition
                            let thresh = (self.width_animator.value
                                / self.fractal_matrix.width() as f64)
                                * 0.1;
