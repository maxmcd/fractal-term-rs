...
use fract::constants;
use fract::exposure::{ExposureInfo, ExposureUtil};
...
pub trait View {
    ...
    fn do_update(&mut self) {
        ...
        if self.width_animator().value > dw {
            ...
            match self.width_animator().anim() {
                &Anim::ScaleVelocity { scale_velocity, .. } => {
                    self.width_animator()
                        .set_scale_velocity(scale_velocity.abs() * -0.25); // muted bounce
                }
                _ => {}
            }
        }
        ...
        let (f, c) = if self.use_exposure() {
            (
                self.exposure_info().floor as f64,
                self.exposure_info().ceil as f64,
            )
        } else {
    ...
    fn calculate(&mut self) {
        ...
        if dirty1 {
            ...
            let info =
                ExposureUtil::calc(&self.fractal_matrix(), self.specs().max_val, 0.040, 0.010);
        ...
        let bias = if self.use_exposure() {
            if self.exposure_info().bias > 0.0 {
                self.exposure_info().bias
            } else {
                0.0
            }
        } else {
        ...
        let v = vec![
            self.exposure_floor_animator().value,
            self.exposure_ceil_animator().value,
        ];
    ...
    fn anim_to_home(&mut self) {
        ...
        self.rotation_animator().set_anim(Anim::Target {
            target: 0.0,
            coefficient: constants::TARGET_COEF,
            epsilon: None,
        });
    ...
    fn calc_index_matrix(&mut self) {
        assert!(
            self.fractal_matrix().width() == self.index_matrix().width()
